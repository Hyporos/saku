    ...
    function getCommandInfo(type) {
      if (type === "description") {
        ...
        switch (selectedCommand) {
          ...
          case "weekly":
            return "View weekly guild culvert statistics and score graphs. Use the `stats` subcommand to view totals, averages, and percentiles for a given week, or the `graph` subcommand to see the guild's score progression over time. Bee-only.";
          case "roll":
    ...
    const help = new EmbedBuilder()
      ...
      .addFields({
        ...
        value: "`gpq`, `profile`, `graph`, `graphcolor`, `rankings`" +
          (isBee
            ? ", `link`, `unlink`, `rename`, `changeid`, `correct`, `exception`, `scan`, `culvertping`, `finalize`, `wos`, `export`, `weekly`"
            : ""),
      })
